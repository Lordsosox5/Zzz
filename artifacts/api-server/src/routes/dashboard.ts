...
router.get("/dashboard/stats", async (_req, res): Promise<void> => {
  try {
    ...
    const [
      ...
      supabase.from("appointments").select("*", { count: "exact", head: true }).gte("scheduled_at", today.toISOString()).lte("scheduled_at", todayEnd.toISOString()),
      ...
      supabase.from("appointments").select("*", { count: "exact", head: true }).eq("status", "completed").gte("scheduled_at", today.toISOString()),
    ...
    const lowStock = (allDrugs ?? []).filter(d => Number(d.stock_quantity) <= Number(d.min_stock_level)).length;
