...
router.get("/drugs", async (req, res): Promise<void> => {
  ...
  try {
    if (params.data.lowStock) {
      ...
      const lowStock = allDrugs.filter((d) => Number(d.stockQuantity) <= Number(d.minStockLevel));
...
router.post("/drugs", async (req, res): Promise<void> => {
  ...
  try {
    const [row] = await db.insert(drugsTable).values({
      ...parsed.data,
      unitPrice: parsed.data.unitPrice?.toFixed(2) ?? "0.00",
    }).returning();
    res.status(201).json(formatDrug(row));
  } catch (err: unknown) {
    res.status(500).json({ error: err instanceof Error ? err.message : "Internal server error" });
  }
});
...
router.patch("/drugs/:id", async (req, res): Promise<void> => {
  ...
  try {
    const [row] = await db.update(drugsTable).set(updates).where(eq(drugsTable.id, params.data.id)).returning();
    if (!row) { res.status(404).json({ error: "Drug not found" }); return; }
    res.json(formatDrug(row));
  } catch (err: unknown) {
    res.status(500).json({ error: err instanceof Error ? err.message : "Internal server error" });
  }
});
