...
router.post("/appointments", async (req, res): Promise<void> => {
  ...
  try {
    ...
    const [row] = await db.insert(appointmentsTable).values(data).returning();
...
router.get("/appointments/:id", async (req, res): Promise<void> => {
  ...
  try {
    const [row] = await db.select().from(appointmentsTable).where(eq(appointmentsTable.id, params.data.id)).limit(1);
...
router.patch("/appointments/:id", async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (parsed.data.scheduledAt) updates.scheduledAt = new Date(parsed.data.scheduledAt as string);
    const [row] = await db.update(appointmentsTable).set(updates).where(eq(appointmentsTable.id, params.data.id)).returning();
    if (!row) { res.status(404).json({ error: "Appointment not found" }); return; }
    res.json(await enrichAppointment(row));
  } catch (err: unknown) {
    res.status(500).json({ error: err instanceof Error ? err.message : "Internal server error" });
