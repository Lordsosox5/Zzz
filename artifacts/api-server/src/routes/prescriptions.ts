...
router.get("/prescriptions", async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (params.data.patientId) {
      ...
      const rows = await db.select().from(prescriptionsTable).where(inArray(prescriptionsTable.patientId, allowedPatientIds));
      ...
      res.json(rows.map(r => ({ ...r, patientName: patientMap[r.patientId] ?? null, prescriberName: null })));
      return;
    }
    if (params.data.status) conditions.push(eq(prescriptionsTable.status, params.data.status));

    const rows = conditions.length > 0
      ? await db.select().from(prescriptionsTable).where(conditions.length === 1 ? conditions[0] : conditions.reduce((a, b) => a && b as any))
...
router.post("/prescriptions", async (req, res): Promise<void> => {
  ...
  try {
    const rows = await db.insert(prescriptionsTable).values({ ...parsed.data, prescriberId: 1 }).returning();
...
router.patch("/prescriptions/:id", async (req, res): Promise<void> => {
  ...
  if (!parsed.success) { res.status(400).json({ error: parsed.error.message }); return; }
  try {
    const rows = await db.update(prescriptionsTable).set(parsed.data).where(eq(prescriptionsTable.id, params.data.id)).returning();
    if (!rows[0]) { res.status(404).json({ error: "Prescription not found" }); return; }
    res.json({ ...rows[0], patientName: null, prescriberName: null });
  } catch (err) {
    res.status(500).json({ error: String(err) });
